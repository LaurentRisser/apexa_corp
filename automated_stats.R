...
input <- read.csv(file_name, sep = ",", header = TRUE, colClasses = c("character","character","character"), na.strings = NA)[,c(2,8,12)]
...
colnames(output) <- c(unique(input$Type))
...
write.table(output, file=paste("stats-",current_date,".csv", sep=""), sep = ",", quote = FALSE, row.names = TRUE, col.names = NA)
}